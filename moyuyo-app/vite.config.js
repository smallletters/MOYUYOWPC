...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    resolve: {
      alias: {
        '@': '/src',
        // three.js 子路径解析：uni-app 的 vite 默认不会扫 node_modules/three/examples/，
        // 手动把 examples/jsm/* 重定向到本地 three 包目录（用绝对路径，避免不同 cwd 解析失败）
    ...
    // dev server 配置：fs.allow 允许访问 node_modules（用于 dynamic import 加载 three 子模块）
    server: {
      // 允许访问项目根 + node_modules（解决 dynamic import 加载 three 子模块被 fs 限制的问题）
      fs: {
