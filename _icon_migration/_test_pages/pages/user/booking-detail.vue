<template>
  <view class="page">
    ...
    <view v-else class="content">
      <view class="card">
        <view class="row"><text class="label">预约编号</text><text class="value">#{{ detail.id }}</text></view>
        <view class="row"><text class="label">类型</text><text class="value">{{ detail.serviceType || detail.type || '—' }}</text></view>
        <view class="row"><text class="label">预约时间</text><text class="value">{{ formatTime(detail.bookingTime || detail.scheduledAt) }}</text></view>
        <view v-if="detail.location" class="row"><text class="label">地点</text><text class="value">{{ detail.location }}</text></view>
        <view v-if="detail.contact" class="row"><text class="label">联系方式</text><text class="value">{{ detail.contact }}</text></view>
        <view class="row"><text class="label">状态</text><text class="value status" :class="'status-' + (detail.status || 'PENDING')">{{ statusLabel(detail.status) }}</text></view>
        <view v-if="detail.remark" class="row"><text class="label">备注</text><text class="value">{{ detail.remark }}</text></view>
      </view>

      <view class="actions" v-if="detail.status !== 'CANCELLED' && detail.status !== 'COMPLETED'">
...
.card { background: var(--color-surface); border-radius: 16rpx; padding: 16rpx 24rpx; }
.row { display: flex; justify-content: space-between; padding: 16rpx 0; border-bottom: 1rpx solid var(--color-divider); }
.row:last-child { border-bottom: none; }
.label { font-size: 26rpx; color: var(--color-text-tertiary); }
.value { font-size: 26rpx; color: var(--color-text); }
.status { padding: 4rpx 12rpx; border-radius: 999rpx; font-size: 22rpx; background: #f0f0f0; }
.status-CONFIRMED { background: #e6f7ff; color: var(--color-primary); }
.status-CANCELLED { background: #ffecec; color: #c0392b; }
.status-COMPLETED { background: #f0f9eb; color: #67c23a; }
.actions { margin-top: 24rpx; }
.btn { height: 88rpx; border-radius: 44rpx; display: flex; align-items: center; justify-content: center; border: 1rpx solid #c0392b; color: #c0392b; font-size: 28rpx; }
</style>